...
export class EnvModule extends ConfigurableModuleClass {
  ...
  private static load(env?: string) {
    ...
    const configFilePaths = configs
      .filter(Boolean)
      .map(String)
      .map((name) => path.resolve(path.join(process.cwd(), root), name + extension))
    ...
    for (const configFilePath of configFilePaths) config = merge(config, readJson(configFilePath))
